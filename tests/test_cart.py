...
from data.cart_data import (ACCESS_CART_PAGE_WITHOUT_LOGIN_ERROR,
                            CART_ITEM_DATA, CART_ITEMS_DATA, DESCRIPTION,
                            QUANTITY, YOUR_CART)
...
def test_verify_cart_list_items(empty_cart_page: CartPage) -> None:
    expect(empty_cart_page.quantity).to_have_text(QUANTITY)
    expect(empty_cart_page.description).to_have_text(DESCRIPTION)
